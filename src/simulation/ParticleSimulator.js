...
const SIM_SETTINGS = {
    ...
    orbitSpeed: { x:Math.PI/90, y:Math.PI/60, z:Math.PI/30 },
    ...
    focusZ: 0.4,                  // camera focus distance
...
for (const typeKey in INITIAL_COUNTS) {
    ...
    for (let i = 0; i < INITIAL_COUNTS[typeKey]; i++) {
        ...
        const mass = Math.random()*(type.massRange[1]-type.massRange[0]) + type.massRange[0];
        particles.push({
            el,
            name: type.name,
            color: type.color,
...
// === INITIAL PROTON–NEUTRON BINDING (50%) ===
(() => {
    const protons  = particles.filter(p => p.name === 'proton');
    const neutrons = particles.filter(p => p.name === 'neutron');
    const totalPairs = Math.min(protons.length, neutrons.length);
    const bindCount  = Math.floor(totalPairs * 0.5);
    for (let i = 0; i < bindCount; i++) {
        const p = protons.splice(Math.floor(Math.random()*protons.length),1)[0];
        const n = neutrons.splice(Math.floor(Math.random()*neutrons.length),1)[0];
        p.boundTo = n; n.boundTo = p;

        // position n at ~0.9·bindingDistance in random 3D dir
        ...
        // match velocities so they start "at rest"
...
for (const e of particles) {
    ...
    // radial vector
    const rx = e.x - nearest.x,
          ry = e.y - nearest.y,
          rz = e.z - nearest.z;

    // random perpendicular direction via cross(r, w)
    ...
    // apply global electronOrbitScale
...
// right-click → create black-hole
container.addEventListener('contextmenu', evt => {
    evt.preventDefault();
    blackHoles.push({
        x: evt.clientX, y: evt.clientY, z: SIM_SETTINGS.focusZ,
...
container.addEventListener('wheel', evt => {
    ...
    // Determine scroll direction (positive for scroll up/zoom in, negative for scroll down/zoom out)
    const scrollDirection = Math.sign(evt.deltaY);
    
    // Update the focusZ value with smooth damping
    SIM_SETTINGS.focusZ += scrollDirection * SIM_SETTINGS.focusZSpeed;
    
    // Clamp to min/max values
    SIM_SETTINGS.focusZ = Math.max(SIM_SETTINGS.minFocusZ, 
                          Math.min(SIM_SETTINGS.maxFocusZ, SIM_SETTINGS.focusZ));
}, { passive: false });
...
function update(){
    const {
        ...
        electronOrbitScale, depthScale, nearRenderZ, farRenderZ, maxRenderBlur,
        bhGravity, bhLifetime, scaleFactor, focusZ
    } = SIM_SETTINGS;
    ...
    for (const a of particles){
        ...
        for (let ox=-1; ox<=1; ox++) for (let oy=-1; oy<=1; oy++) for (let oz=-1; oz<=1; oz++){
            ...
            for (const b of cell){
                ...
                // nuclear Yukawa+repulsion for p↔n
                if ((a.name==='proton'&&b.name==='neutron')||(a.name==='neutron'&&b.name==='proton')){
                    const expT = Math.exp(-nuclearYukawaMu*d);
                    const Fat  = nuclearYukawaStrength*expT*(nuclearYukawaMu*d+1)/(d*d);
                    const Frep=12*nuclearRepulsionA/Math.pow(d,13);
                    const Fn   = Fat - Frep;
        ...
        for (const bh of blackHoles){
            ...
            const Fg = bhGravity*(bh.mass*a.mass)/d2;
        ...
        // relativistic mass for electrons
        let massEff=a.mass;
        if (a.name==='electron'){
            const v2 = a.vx*a.vx + a.vy*a.vy + a.vz*a.vz;
            const β2 = Math.min(v2/(speedOfLight*speedOfLight),0.9999);
            const gamma = 1/Math.sqrt(1-β2);
    ...
    for (const a of particles){
        ...
        if (a.x<-50) a.x=innerWidth+50; if (a.x>innerWidth+50) a.x=-50;
        if (a.y<-50) a.y=innerHeight+50; if (a.y>innerHeight+50) a.y=-50;

        let dx=a.x-cx0, dy=a.y-cy0, dz=(a.z-focusZ)*depthScale;

        // rotate X
        {
            const ca=Math.cos(angles.x), sa=Math.sin(angles.x);
            [dy,dz]=[dy*ca-dz*sa, dy*sa+dz*ca];
        }
        // rotate Y
        {
            const ca=Math.cos(angles.y), sa=Math.sin(angles.y);
            [dx,dz]=[dx*ca-dz*sa, dx*sa+dz*ca];
        }
        // rotate Z
        {
            const ca=Math.cos(angles.z), sa=Math.sin(angles.z);
            [dx,dy]=[dx*ca-dy*sa, dx*sa+dy*ca];
        }

        const rx = cx0 + dx,
              ry = cy0 + dy,
              zNow = focusZ + dz/depthScale;

        if (zNow<nearRenderZ||zNow>farRenderZ){
            a.el.style.display='none'; continue;
        }

        const zd=zNow-focusZ, za=Math.abs(zd),
        ...
        // apply zoom factor
        const scale = (SIM_SETTINGS.scaleFactor / zNow).toFixed(2);
        const bright = zd<0
            ? 1+Math.min(0.2,-zd)
            : 1-Math.min(0.5,zd);
        const op = Math.max(0.3, Math.min(1, 1.1-za)).toFixed(2);

        // highlight bound pairs - replace border with hue-rotate
        const hueShift = a.boundTo ? 'hue-rotate(-10deg)' : '';

        a.el.style.border = 'none'; // remove border completely
        a.el.style.display = '';
